...
class Canvas(QtWidgets.QWidget):
    """Canvas widget for drawing and editing shapes on images."""
    ...
    def _get_osam_session(self):
        """Get or create OSAM session for SAM-based annotation."""
        try:
            from labelvid._automation import OsamSession

            if OsamSession is None:
                logger.warning("OSAM not available - please install: pip install osam")
                return None
            ...
            logger.warning("OSAM session error: {}", e)
            return None

    def _update_shape_with_ai(
        self, points: list[QPointF], point_labels: list[int], shape: Shape
    ) -> None:
        """Update shape using AI (SAM) prediction."""
        try:
            import imgviz
            import osam

            session = self._get_osam_session()
            if session is None:
                return

            image: np.ndarray = utils.img_qt_to_arr(img_qt=self.pixmap.toImage())
    ...
    def mouseReleaseEvent(self, a0: QtGui.QMouseEvent) -> None:
        if a0.button() == Qt.RightButton:
            ...
            self.restoreCursor()
            if (
                not menu.exec_(self.mapToGlobal(a0.pos()))
        ...
        if self.movingShape and self.hShape:
            ...
            # Check if we have backups and the index is valid
            if (self.shapesBackups and 
                len(self.shapesBackups[-1]) > index and
                self.shapesBackups[-1][index].points != self.shapes[index].points):
                self.storeShapes()
    ...
    def intersectingEdges(self, point1, point2, points):
        ...
        for i in range(4):
            ...
            if 0 <= ua <= 1 and 0 <= ub <= 1:
                ...
                d = utils.distance(m - QPointF(x2, y2))
                yield d, i, (x, y)
    ...
    def loadPixmap(self, pixmap, clear_shapes=True):
        ...
        self._pixmap_hash = hash(
            utils.img_qt_to_arr(img_qt=self.pixmap.toImage()).tobytes()
        )
        if clear_shapes:
            self.shapes = []
        self.update()
...
def _update_shape_with_ai_response(response, shape: Shape, createMode: str) -> None:
    """Update shape with AI response from SAM."""
    ...
    try:
        ...
        from labelvid._automation import polygon_from_mask
        ...
        logger.warning("AI annotation requires imgviz package")
    except Exception as e:
        logger.error(f"Failed to update shape with AI response: {e}")
